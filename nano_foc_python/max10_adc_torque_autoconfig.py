...
def main():
    ...
    nano_write_reg(0x23, 0x0000)    #set hall offset to 0
    nano_write_reg(0x54, 0x0C00)    #set flux target
    ...
    nano_write_reg(0x16, 0x1009)    #enable interrupt output, toggle mode, at end of calculation, PWM freq / 10
    ...
    #apply the offset
    nano_write_reg(0x23, phi_dif_avg)

    #switch to torque mode
    nano_write_reg(0x54, 0x0000)    #set flux target
    nano_write_reg(0x10, 0x0003)    #phi_source = 2'b11 (extpol_phi), vd_vq_source = 1'b0 (internal), vu_vv_vw_source = 1'b0 (internal)
    nano_write_reg(0x5A, 0x044C)    #set torque target
    nano_write_reg(0x31, 0x0000)    #set ol target velocity to 0

    #setup RTMI again
    rtmi_responses[0].queue.clear()
    rtmi_responses[1].queue.clear()
    nano_write_reg(0x7FF4, 0x0053)  #RTMI channel 0 set to torque
    nano_configure_rtmi(6, 0, 1, 0, 1, 512, 0)  #unconditional, ch0, 1 channels, not continuous, triggered, 512 samples, 0 threshold
    ...
    if(power_off):
        ...
        nano_write_reg(0x5A, 0x0000)    #set torque target to 0
        nano_write_reg(0x00, 0x0000)    #power stage off
