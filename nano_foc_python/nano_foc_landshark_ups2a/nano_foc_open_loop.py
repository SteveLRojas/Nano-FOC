...
def main():
    ...
    nano_write_reg(regs.R_SOURCE_CTRL, 0x0005)    #phi_source = 2'b01 (ol_phi), vd_vq_source = 1'b1 (external), vu_vv_vw_source = 1'b0 (internal)
    ...
    nano_write_reg(regs.R_OL_ACCELERATION, 1)    #ol_acceleration
    nano_write_reg(regs.R_OL_TARGET_VELOCITY, 20)    #ol_target_velocity
    nano_write_reg(regs.R_V_D_EXT, 700)    #v_d_ext
    ...
    nano_write_reg(regs.R_INT_OUT_CTRL, 0x181D)    #enable interrupt output, at start of calculation, toggle mode, PWM freq / 30
    nano_write_reg(regs.FR_RTMI_CHANNEL_0, regs.R_I_U_KCL)
    nano_write_reg(regs.FR_RTMI_CHANNEL_1, regs.R_I_V_KCL)
    nano_write_reg(regs.FR_RTMI_CHANNEL_2, regs.R_I_W_KCL)
    nano_configure_rtmi(6, 0, 3, 0, 1, 1024, 0)  #unconditional, ch0, 3 channels, not continuous, triggered, 1024 samples, 0 threshold
    ...
    print(f"Status: 0x{nano_read_reg(regs.R_STATUS):04X}")
    nano_write_reg(regs.R_V_D_EXT, 0x0000)    #v_d_ext = 14'd0
    nano_write_reg(regs.R_STATUS, 0x0000)    #power stage off
    ...
    #sign extend
    num_valid_samples = min(rtmi_responses[0].qsize(), rtmi_responses[1].qsize())
    num_valid_samples = min(num_valid_samples, rtmi_responses[2].qsize())
    current_u = []
    current_v = []
    current_w = []
    for idx in range(num_valid_samples):
        response = rtmi_responses[0].queue[idx]
        response = (response & 0x1FFF) - (response & 0x2000)
        current_u.append(response)
        response = rtmi_responses[1].queue[idx]
        response = (response & 0x1FFF) - (response & 0x2000)
        current_v.append(response)
        response = rtmi_responses[2].queue[idx]
        response = (response & 0x1FFF) - (response & 0x2000)
