...
def main():
	...
	norm_ol_phi = [element / 16384 for element in list(nano.rtmi_responses[0].queue)]
	norm_hall_sector = [element / 6 for element in list(nano.rtmi_responses[1].queue)]
	...
	nano.flush_rtmi()
	nano.write_reg(nano.regs.FR_RTMI_CHANNEL_0, nano.regs.R_OL_PHI)
	nano.write_reg(nano.regs.FR_RTMI_CHANNEL_1, nano.regs.R_HALL_PHI)
	nano.configure_rtmi(6, 0, 2, 0, 1, 1024, 0)  #unconditional, ch0, 2 channels, not continuous, triggered, 256 samples, 0 threshold
	...
	plt.plot(nano.rtmi_responses[1].queue, label='HALL_PHI')
	plt.legend()
	plt.show()

	#spin the motor in reverse
	nano.write_reg(nano.regs.R_OL_TARGET_VELOCITY, -256 & 0xFFFF)
	time.sleep(8)
	...
	nano.flush_rtmi()
	nano.write_reg(nano.regs.FR_RTMI_CHANNEL_0, nano.regs.R_OL_PHI)
	nano.write_reg(nano.regs.FR_RTMI_CHANNEL_1, nano.regs.R_EXTPOL_PHI)
	nano.configure_rtmi(6, 0, 2, 0, 1, 1024, 0)  #unconditional, ch0, 2 channels, not continuous, triggered, 256 samples, 0 threshold
	...
	nano.write_reg(nano.regs.R_OL_TARGET_VELOCITY, 0x0000)    #set target velocity to 0
	nano.write_reg(nano.regs.R_STATUS, 0x0000)    #power stage off
