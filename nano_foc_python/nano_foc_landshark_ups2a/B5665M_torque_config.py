...
def main():
	...
	time.sleep(0.25)
	nano.write_reg(nano.regs.R_OL_ACCELERATION, 3)    #ol_acceleration
	nano.write_reg(nano.regs.R_OL_TARGET_VELOCITY, 200)    #ol_target_velocity

	#configure ABZ decoder
	nano.write_reg(nano.regs.R_ENC_OFFSET, 0)
	abz_inv_dir = 1
	abz_combined_z_pulse = 0
	abz_clear_en = 1
	nano.write_reg(nano.regs.R_ABZ_CONF, (abz_inv_dir << 2) | (abz_combined_z_pulse << 1) | abz_clear_en)
	abz_num_lines = 256
	abz_cpr = abz_num_lines * 4
	abz_num_pole_pairs = 6
	abz_cpr_inv = abz_num_pole_pairs * 2**28 // abz_cpr
	nano.write_reg(nano.regs.R_ABZ_CPR_INV_L, abz_cpr_inv & 0xFFFF)
	nano.write_reg(nano.regs.R_ABZ_CPR_INV_H, abz_cpr_inv >> 16)
	nano.write_reg(nano.regs.R_ABZ_MAX_COUNT, abz_cpr - 1)
	time.sleep(1)
	...
	int_out_ctrl = int_out_div | (int_out_mode << 10) | (int_out_pol << 11) | (int_out_en << 12)
	nano.write_reg(nano.regs.R_INT_OUT_CTRL, int_out_ctrl)
	nano.write_reg(nano.regs.FR_RTMI_CHANNEL_0, nano.regs.R_OL_PHI)
	nano.write_reg(nano.regs.FR_RTMI_CHANNEL_1, nano.regs.R_ENC_PHI)
	nano.write_reg(nano.regs.FR_RTMI_CHANNEL_2, nano.regs.R_ABZ_PHI)
	nano.configure_rtmi(6, 0, 3, 0, 1, 2048, 0)  #unconditional, ch0, 3 channels, not continuous, triggered, 2048 samples, 0 threshold
	...
	for idx in range(rtmi_num_samples):
		sext_dif = nano.rtmi_responses[0].queue[idx] - nano.rtmi_responses[1].queue[idx]
	...
	#apply the offset
	nano.write_reg(nano.regs.R_ENC_OFFSET, phi_dif_avg & 0xFFFF)
	...
	plt.plot(nano.rtmi_responses[1].queue, label='SELECTED_PHI')
	plt.legend()
	plt.show()

	#switch to torque mode
	nano.write_reg(nano.regs.R_FLUX_TARGET, 0x0000)    #set flux target
	nano.write_reg(nano.regs.R_STATUS, 0x0000)    #power stage off
	nano.write_reg(nano.regs.R_OL_TARGET_VELOCITY, 0x0000)    #set ol target velocity to 0
	vu_vv_vw_source = 0 #internal
	vd_vq_source = 0    #internal
	phi_source = 5  #enc_phi, abz_phi selected
	nano.write_reg(nano.regs.R_SOURCE_CTRL, (vu_vv_vw_source << 4) | (vd_vq_source << 3) | phi_source)
	time.sleep(0.1)
	nano.write_reg(nano.regs.R_STATUS, 0x0008)    #power stage on
	nano.write_reg(nano.regs.R_TORQUE_TARGET, 1500)    #set torque target
