...
def main():
    ...
    plat.nano_write_reg(regs.R_FLUX_TARGET, 0x0000)    #set flux target
    plat.nano_write_reg(regs.R_STATUS, 0x0000)    #power stage off
    plat.nano_write_reg(regs.R_OL_TARGET_VELOCITY, 0x0000)    #set ol target velocity to 0
    vu_vv_vw_source = 0 #internal
    vd_vq_source = 0    #internal
    ...
    time.sleep(0.1)
    plat.nano_write_reg(regs.R_STATUS, 0x0008)    #power stage on
    plat.nano_write_reg(regs.R_TORQUE_TARGET, 1500)    #set torque target
    ...
    int_out_ctrl = int_out_div | (int_out_mode << 10) | (int_out_pol << 11) | (int_out_en << 12)
    plat.nano_write_reg(regs.R_INT_OUT_CTRL, int_out_ctrl)
    plat.nano_write_reg(regs.FR_RTMI_CHANNEL_0, regs.R_TORQUE)
    plat.nano_write_reg(regs.FR_RTMI_CHANNEL_1, regs.R_FLUX)
    plat.nano_configure_rtmi(6, 0, 2, 0, 1, 2048, 0)  #unconditional, ch0, 2 channels, not continuous, triggered, 2048 samples, 0 threshold
    ...
    rtmi_num_samples = min(plat.rtmi_responses[0].qsize(), plat.rtmi_responses[1].qsize())
    for idx in range(rtmi_num_samples):
        plat.rtmi_responses[0].queue[idx] = (plat.rtmi_responses[0].queue[idx] & 0x1FFF) - (plat.rtmi_responses[0].queue[idx] & 0x2000)
        plat.rtmi_responses[1].queue[idx] = (plat.rtmi_responses[1].queue[idx] & 0x1FFF) - (plat.rtmi_responses[1].queue[idx] & 0x2000)

    #plot RTMI responses
    plt.plot(plat.rtmi_responses[0].queue, label='TORQUE')
    plt.plot(plat.rtmi_responses[1].queue, label='FLUX')
    plt.legend()
    plt.show()
